...
#script based on install_mint_1901_tessa.sh 
...
uninstall_repos() {
	sudo rm -f /etc/apt/sources.list.d/gstffmpeg-keep.list
  sudo apt-key del ED8E640A
	sudo add-apt-repository -y -r "deb http://old-releases.ubuntu.com/ubuntu/ wily main universe multiverse"
	sudo apt-get update
}

#there are some mkdirs etc. in this skript...
if [ $UID -eq 0 ]; then
    echo "Abbruch: Dieses Skript darf nicht als root ausgeführt werden."
    exit 255
fi
...
echo "Installscript OTRV++ für Linux Mint 19.2 Tina"
echo ""
...
if [ "$DISTRIB_DESCRIPTION" == "Linux Mint 19.2 Tina" ]
  then
	# Paketliste updaten und Abhängigkeiten installieren
    sudo tee <<'EOF' /etc/apt/sources.list.d/gstffmpeg-keep.list > /dev/null
...
EOF
  ...
  # Avidemux 2.5 cli fürs Schneiden von divx - Smartmkvmerge kann divx ebenfalls schneiden, deshalb nicht unbedingt benötigt
	...
	# wine für Virtualdub
	# folgende Zeile mit # auskommentieren, wenn nicht benötigt
	sudo apt-get -y install wine-stable winetricks wine32-development
